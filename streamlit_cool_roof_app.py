...
st.set_page_config(page_title="Bolix RX – Cool Roof Calculator", page_icon="🌞", layout="wide")

st.title("🌞 Bolix RX – Cool Roof Calculator")
...
# --- Updated Graph (Cumulative Savings) ---
df = pd.DataFrame({
    "Rok": years,
    "Energia (kWh)": cum_kwh,
    "Koszty (zł)": cum_cost,
...
fig = px.line(
    df, x="Rok", y=df.columns[1:], 
    markers=True, 
    title="20-letnie oszczędności – kumulacja"
)
